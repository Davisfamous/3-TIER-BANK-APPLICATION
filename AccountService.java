...
import com.apex.bank.exception.MysteryRewardQualifiedException;
...
public class AccountService {
    ...
    public AccountResponse create(CreateAccountRequest request) {
        ...
        BigDecimal overdraftLimit = type == com.apex.bank.Enums.AccountType.CURRENT ? request.getOverdraftLimit() : null;
        BigDecimal openingBalance = request.getOpeningBalance() == null ? ZERO : request.getOpeningBalance();
        if (openingBalance.signum() < 0) {
            throw new IllegalArgumentException("openingBalance must be >= 0");
        }

        User user = userRepository.findByFirstNameIgnoreCaseAndLastNameIgnoreCase(request.getFirstName(), request.getLastName())
                .map(existing -> {
                ...
                .orElseGet(() -> userRepository.save(
                        new User(request.getFirstName(), request.getLastName(), passwordEncoder.encode(request.getPassword()))
                ));
    ...
    @Transactional(noRollbackFor = MysteryRewardQualifiedException.class)
    ...
    public void transfer(Long userId, Long fromAccountId, Long toAccountId, BigDecimal amount) {
        ...
        if ("SAVINGS".equalsIgnoreCase(from.getAccountType()) && newFromBalance.signum() < 0) {
            throw new RuntimeException("Savings account cannot go below zero");
        }
        if ("CURRENT".equalsIgnoreCase(from.getAccountType()) && from.getOverdraftLimit() != null) {
            BigDecimal minBalance = from.getOverdraftLimit().negate();
            if (newFromBalance.compareTo(minBalance) < 0) {
                throw new RuntimeException("Overdraft limit exceeded");
            }
        }
    ...
    public List<TransactionResponse> getTransactions(Long accountId, Long userId) {
        ...
        return accountTransactionRepository.findByAccountIdOrderByCreatedAtDesc(accountId).stream()
                .map(tx -> new TransactionResponse(tx.getId(), tx.getType().name(), tx.getAmount(), tx.getCreatedAt(), tx.getReference()))
                .toList();
    }
    ...
    private void checkSavingsReward(Account account) {
        if (!"SAVINGS".equalsIgnoreCase(account.getAccountType())) {
            return;
        }

        BigDecimal threshold = account.getNextRewardThreshold() == null
                ? new BigDecimal("1000")
                : account.getNextRewardThreshold();

        if (!account.isRewardEligible() && account.getBalance().compareTo(threshold) >= 0) {
            account.setRewardEligible(true);
        }

        if (account.isRewardEligible() && account.getBalance().compareTo(threshold) >= 0) {
            account.setRewardEligible(false);
            account.setNextRewardThreshold(threshold.add(new BigDecimal("1000")));
            accountRepository.save(account);
            throw new MysteryRewardQualifiedException(
                    "Customer qualifies for mystery reward: " + rewardService.getReward());
        }
    }
