...
import { getBudgetNotes, createBudgetNote, generateBudgetNote } from "./api/budgetApi";
...
export default function AccountDetails() {
  ...
  // Load budget notes for user
  useEffect(() => {
    if (!authUser?.id) return;
    getBudgetNotes(authUser.id)
      .then((res) => setBudgetNotes(res.data || []))
      .catch(() => setBudgetError("Failed to load budget notes"));
  }, [authUser]);
        ...
        <>
          ...
          {activeAccount && (
            <div className="account-details-card">
              ...
              <div className="budget-notes-card" style={{ marginTop: 32 }}>
                <h3>Budget Notes</h3>
                <form
                  onSubmit={async (e) => {
                    e.preventDefault();
                    ...
                    setBudgetError("");
                    try {
                      const res = await createBudgetNote({
                        userId: authUser.id,
                        note: budgetInput
                      });
                      setBudgetNotes((prev) => [res.data, ...prev]);
                      setBudgetInput("");
                    } catch (err) {
                      setBudgetError(getErrorMessage(err, "Failed to save budget note"));
                    } finally {
                  ...
                  style={{ marginTop: 12 }}
                >
                  <label className="budget-field-label" htmlFor="budgetPromptDetails">
                    AI Prompt
                  </label>
                  <textarea
                    id="budgetPromptDetails"
                    value={budgetPrompt}
                    onChange={(e) => setBudgetPrompt(e.target.value)}
                    placeholder="Describe your income, expenses, debts, savings goal, or budgeting challenge..."
                    rows={3}
                    style={{ width: "100%", marginBottom: 8 }}
                  />
                  <button
                    type="button"
                    className="budget-note-ai-btn"
                    disabled={budgetGenerating || !budgetPrompt.trim()}
                    onClick={async () => {
                      setBudgetGenerating(true);
                      setBudgetError("");
                      try {
                        const res = await generateBudgetNote({
                          userId: authUser.id,
                          note: budgetPrompt
                        });
                        setBudgetInput(res.data?.generatedNote || "");
                      } catch (err) {
                        setBudgetError(getErrorMessage(err, "Failed to generate budget note"));
                      } finally {
                        setBudgetGenerating(false);
                      }
                    }}
                  >
                    {budgetGenerating ? "Generating..." : "Generate with AI"}
                  </button>
                  <label className="budget-field-label" htmlFor="budgetNoteDetails">
                    Budget Note
                  </label>
                  <textarea
                    id="budgetNoteDetails"
                    value={budgetInput}
                    onChange={(e) => setBudgetInput(e.target.value)}
                    placeholder="Your generated budget note will appear here. You can edit it before saving."
                    rows={6}
                    style={{ width: "100%", marginBottom: 8 }}
                    required
                  />
                  <div className="budget-note-actions">
                    <button type="submit" disabled={budgetSaving || !budgetInput.trim()}>
                      {budgetSaving ? "Saving..." : "Save Note"}
                    </button>
                  </div>
                </form>
                {budgetError && <div className="budget-error">{budgetError}</div>}
                <ul className="budget-notes-list" style={{ marginTop: 18 }}>
