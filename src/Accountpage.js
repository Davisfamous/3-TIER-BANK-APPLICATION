...
import { createBudgetNote, generateBudgetNote, getBudgetNotes } from "./api/budgetApi";
...
export default function AccountDashboard() {
  ...
  useEffect(() => {
    const fetchBudgetNotes = async () => {
      if (!authUser?.id) return;
      try {
        const res = await getBudgetNotes(authUser.id);
        setBudgetNotes(res.data || []);
      } catch (e) {
        setBudgetError("Failed to load budget notes");
      }
    };
    fetchBudgetNotes();
  }, [authUser]);
  ...
  return (
    <div className="dashboard">
      <div className="dashboard-container">
        ...
        <div className="budget-notes-card">
          <h3>Budget Notes</h3>
          <form
            onSubmit={async (e) => {
              e.preventDefault();
            ...
            style={{ marginTop: 16 }}
          >
            <label className="budget-field-label" htmlFor="budgetPrompt">
              AI Prompt
            </label>
            <textarea
              id="budgetPrompt"
            ...
            <label className="budget-field-label" htmlFor="budgetNote">
              Budget Note
            </label>
            <textarea
              id="budgetNote"
